...
// **************************************************************************************
	...
	*pBeholder3 = *pBeholder;
	...
	{
		if (animation_to_play == 1 && button_pressed)
		{
			button_pressed = false;
			pCar->bUse_RGBA_colour = true;
			pCar2->bUse_RGBA_colour = true;
			pBeholder->bUse_RGBA_colour = true;
			pBeholder2->bUse_RGBA_colour = true;
			pBeholder2->position.y = -50.f;
			pBeholder3->bUse_RGBA_colour = true;
			pCar->position = glm::vec3(0, 10, 0);
			pCar->Animation.Speed = 0.05f;
			pCar->Animation.AnimationTime = 0.f;
			pBeholder->Enabled = true;
			pBeholder->Animation.IsLooping = false;
			pBeholder->Animation.IsPlaying = true;
			pBeholder->Animation.Speed = 0.05f;
			pBeholder->Animation.AnimationType = "beholder_Animation1_EaseIn";
			pBeholder->Animation.AnimationTime = 0.f;

			Watcher* CarDrive = new Watcher;
			CarDrive->object = pCar;
			CarDrive->animation_name_to_watch = "beholder_Animation1_EaseIn";
			CarDrive->animation_name_to_play = "car_Animation1_EaseInOut";
			CarDrive->condition = eStatus::FINISHED;

			Watcher* beholderExitCar = new Watcher;
			beholderExitCar->object = pBeholder;
			beholderExitCar->animation_name_to_watch = "car_Animation1_EaseInOut";
			beholderExitCar->animation_name_to_play = "beholder_ExitCar_EaseOut";
			beholderExitCar->condition = eStatus::FINISHED;

			pCar2->Enabled = true;
			pCar2->Animation.IsLooping = false;
			pCar2->Animation.IsPlaying = true;
			pCar2->Animation.Speed = 0.05f;
			pCar2->Animation.AnimationType = "SecondCar_NoEase";
			pCar2->Animation.AnimationTime = 0.f;

			event_handler->watching.push_back(CarDrive);
			event_handler->watching.push_back(beholderExitCar);
			car_animations->Initialize();

		}
		if (animation_to_play == 2 && button_pressed)
		{
			button_pressed = false;
			pCar->Enabled = false;
			pBeholder->Enabled = false;
			pCar2->Enabled = false;
			pCar->bUse_RGBA_colour = true;
			pCar2->bUse_RGBA_colour = true;
			pBeholder->bUse_RGBA_colour = true;
			...
			pBeholder3->bUse_RGBA_colour = true;
			//Beholder1_Eat
			pBeholder2->Enabled = true;
			pBeholder2->Animation.IsLooping = false;
			pBeholder2->Animation.IsPlaying = true;
			pBeholder2->Animation.Speed = 0.05f;
			pBeholder2->Animation.AnimationType = "Beholder1_Eat";
			pBeholder2->Animation.AnimationTime = 0.f;
			//Beholder_Disappear
			Watcher* beholderDisappear = new Watcher;
			beholderDisappear->object = pBeholder;
			beholderDisappear->animation_name_to_watch = "Beholder1_Eat";
			beholderDisappear->animation_name_to_play = "Beholder_Disappear";
			beholderDisappear->condition = eStatus::FINISHED;
			event_handler->watching.push_back(beholderDisappear);
			car_animations->Initialize();
		}
		if (animation_to_play == 3 && button_pressed)
		{
			button_pressed = false;
			pCar->Enabled = false;
			pBeholder->Enabled = false;
			...
			//Beholder_GetBackInCar
			pBeholder2->Enabled = true;
			pBeholder2->Animation.IsLooping = false;
			pBeholder2->Animation.IsPlaying = true;
			pBeholder2->Animation.Speed = 0.05f;
			pBeholder2->Animation.AnimationType = "Beholder_GetBackInCar";
			pBeholder2->Animation.AnimationTime = 0.f;

			Watcher* driveOff = new Watcher;
			driveOff->object = pCar2;
			driveOff->animation_name_to_watch = "Beholder_GetBackInCar";
			driveOff->animation_name_to_play = "SecondCar_DriveOff";
			driveOff->condition = eStatus::FINISHED;
			event_handler->watching.push_back(driveOff);
		...
		if (!pause)
		{
			animation_manager->Update(g_pMeshObjects, 0.1f);
			event_handler->update(0.1f);
		}
		for (cMeshObject* object : g_pMeshObjects)
		{
			if (object->hasPhysicsObject)
			{
				object->position = object->physics_object->position;
			}
		}
